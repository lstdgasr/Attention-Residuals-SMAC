...
SESSION_PREFIX="${SESSION_PREFIX:-rerun_3s5z}"
DRY_RUN="${DRY_RUN:-0}"
RUN_ID="${RUN_ID:-${SESSION_PREFIX}_missing_$(date +%Y%m%d_%H%M%S)}"
...
JOBS=(
  "0|3s5z|qmix_attnres_l2|1|record_attn_weights=True"
  "1|3s5z|qmix_attnres_l2|2|record_attn_weights=True"
  "2|3s5z|qmix_attnres_l2|3|record_attn_weights=True"
  "3|3s5z|qmix_attncomm_l2_other|3|record_comm_attn_weights=True"
  "3|3s5z|qmix_attncomm_l2_self|3|record_comm_attn_weights=True"
)
...
for job in "${JOBS[@]}"; do
  ...
  log_file="$LOG_DIR/${map_name}_${config}_s${seed}_gpu${gpu}_rerun.log"
...
launch_gpu_queue() {
  ...
  for job in "${JOBS[@]}"; do
    ...
    has_job=1
    local log_file="$LOG_DIR/${map_name}_${config}_s${seed}_gpu${gpu}_rerun.log"
    local overrides=""
    if [ -n "$extra" ]; then
      overrides+=" $extra"
    fi
...
echo
echo "Launched 3s5z missing-seed rerun queues."
echo "Monitor with:"
echo "  tmux ls"
echo "  watch -n 5 nvidia-smi"
echo "  tail -f $LOG_DIR/3s5z_qmix_attncomm_l2_self_s3_gpu3_rerun.log"
echo
