...
SESSION_PREFIX="${SESSION_PREFIX:-diag}"
DRY_RUN="${DRY_RUN:-0}"
...
JOBS=(
  "0|5m_vs_6m|qmix_attnres|3"
  "0|3s5z|qmix_attnres_l2|1"
  "0|3s5z|qmix_depth_mlp|1"

  "2|3s5z|qmix_attnres|3"
  "2|5m_vs_6m|qmix_attnres_l2|1"
  "2|5m_vs_6m|qmix_depth_mlp|1"

  "3|5m_vs_6m|qmix_attnres_block|1"
  "3|3s5z|qmix_attnres_l2|2"
  "3|5m_vs_6m|qmix|3"

  "4|5m_vs_6m|qmix_attnres_block|2"
  "4|3s5z|qmix_attnres_l2|3"
  "4|3s5z|qmix|3"

  "5|5m_vs_6m|qmix_attnres_block|3"
  "5|3s5z|qmix_attnres_block|1"
  "5|5m_vs_6m|qmix_attnres_l2|2"

  "6|3s5z|qmix_attnres_block|2"
  "6|5m_vs_6m|qmix_attnres_l2|3"
  "6|5m_vs_6m|qmix_depth_mlp|2"

  "7|3s5z|qmix_attnres_block|3"
  "7|3s5z|qmix_depth_mlp|2"
  "7|3s5z|qmix_depth_mlp|3"
  "7|5m_vs_6m|qmix_depth_mlp|3"
)

manifest="$RESULTS_DIR/run_manifests/${SESSION_PREFIX}_missing_$(date +%Y%m%d_%H%M%S).tsv"
printf "gpu\tmap\tconfig\tseed\tsession\tlog_file\n" > "$manifest"
...
launch_gpu_queue() {
  ...
  local has_job=0
  for job in "${JOBS[@]}"; do
    IFS="|" read -r job_gpu map_name config seed <<< "$job"
    if [ "$job_gpu" != "$gpu" ]; then
      continue
    fi
    has_job=1
    local log_file="$RESULTS_DIR/launcher_logs/${map_name}_${config}_s${seed}_gpu${gpu}.log"
    cmd+=" echo \"[\$(date +%F_%T)] Running gpu=$gpu map=$map_name config=$config seed=$seed\";"
    cmd+=" CUDA_VISIBLE_DEVICES=\"$gpu\" CUDA_DEVICE=\"$gpu\" python src/main.py --config=\"$config\" --env-config=sc2 with env_args.map_name=\"$map_name\" seed=\"$seed\" local_results_path=\"$RESULTS_DIR\" use_cuda=True t_max=\"$T_MAX\" test_nepisode=\"$TEST_NEPISODE\" save_model=\"$SAVE_MODEL\" save_model_interval=\"$SAVE_MODEL_INTERVAL\" 2>&1 | tee \"$log_file\";"
  done
...
echo
echo "Launched missing-task queues."
echo "Monitor with:"
echo "  tmux ls"
echo "  watch -n 5 nvidia-smi"
echo "  tail -f $RESULTS_DIR/launcher_logs/5m_vs_6m_qmix_attnres_s3_gpu0.log"
echo
