...
\input{tables/model_cost_summary}
...
\input{tables/paired_seed_delta}
...
\input{tables/attncomm_5m_extension_results}
...
最后，\code{3s5z} 的 AttnComm sanity check 已补齐，两个通信变体与 AttnRes-L2 均达到 3 个完整 seeds，且 best win 均接近饱和。
因此，本文仍只将 \code{5m\_vs\_6m} 视为主要正向诊断结果；\code{3s5z} 用于说明该扩展没有在 easy heterogeneous map 上出现明显退化，但不作为跨地图稳定提升的强证据。
基于当前结果，本文不再将继续补跑 \code{3s5z} baseline、扩展 AttnComm 到 IQL/VDN，或立即增加新 hard map 作为定稿前的必要实验。
这些扩展更适合作为后续工作：它们可以提高统计可信度或地图覆盖面，但也会显著增加训练成本，并可能引入新的算法变量。
...
这一成本也解释了本文为何将当前实验收束在 3 个 paired seeds 与两张诊断地图上：继续增加 seed4/5 或新增 hard map 可以作为未来验证，但并非支撑本文适配性结论的必要前提。
