...
集中式 critic 和反事实信用分配等研究表明，训练阶段利用更多全局信息是缓解上述问题的重要途径 \cite{foerster2018counterfactual,lowe2017multiagent}。
...
本文围绕 IQL、VDN 和 QMIX 这三类常用 baseline 展开分析 \cite{tan1993multiagent,sunehag2018value,rashid2018qmix}。
...
两张地图均来自 SMAC benchmark，其中 \code{5m\_vs\_6m} 属于 hard 场景，\code{3s5z} 属于 easy 场景 \cite{samvelyan2019starcraft}。
...
这一思想与深层 Transformer 中的残差流、预归一化结构和跨层信息聚合问题密切相关 \cite{vaswani2017attention,xiong2020layernorm,kimi2026attentionresiduals}。
...
第四，如果直接的 depth-wise residual transfer 不稳定，是否说明 attention 思想本身不适合 MARL？
为回答该问题，本文进一步将 selective attention 从 depth dimension 转移到 agent dimension，构建 AttnComm-QMIX-L2 初步扩展，用于检验智能体间通信维度是否更贴合 SMAC 中的协同瓶颈。

本文的贡献可以概括为三点。
首先，本文构建了 AttnRes-RNN 的低侵入式迁移框架，在 PyMARL 中仅修改 recurrent agent 的 hidden-state 后处理模块，并保持 QMIX、IQL 和 VDN 等后端算法主体不变。
其次，本文通过 Full、L2、Block 和 depth-only control 进行受控结构消融，在 \code{5m\_vs\_6m} 与 \code{3s5z} 上分析 final win rate、best win rate、AUC 和训练时间。
已有结果表明，重型 Full/Block AttnRes 直接迁移并不稳定且成本较高，Lightweight AttnRes-L2 在部分 AUC 和 best-win 指标上显示出有限信号，但尚不能支持稳定优于 baseline 的结论。
最后，本文基于结构错位分析提出 AttnComm-QMIX-L2 初步扩展，并在 \code{5m\_vs\_6m} 上观察到比 direct depth-wise transfer 更明确的正向信号。
该结果说明，关键问题并不是 attention 思想本身是否可用，而是迁移结构应放置在更贴近 MARL 协同瓶颈的 agent-wise communication 维度。

需要强调的是，本文并不将当前 AttnRes-RNN 定位为新的性能提升型 MARL 算法。
相反，本文的目标是通过受控结构迁移实验说明：直接迁移 LLM 中用于深层残差信息流的结构到浅层 MARL agent 时，需要谨慎分析其适配性、训练成本和任务瓶颈是否匹配。
