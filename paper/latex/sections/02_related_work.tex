...
\todo{确认 \code{3s5z} 的官方单位组成和 difficulty 标签。}
...
\todo{补充 attention communication MARL 相关引用。}
...
\todo{补充 graph attention MARL 和 transformer-based MARL 相关引用。}

本文当前方法与上述工作存在重要差异。
本文并未直接在智能体之间引入通信 attention，而是首先尝试将 LLM 中的 Attention Residuals 迁移为 agent 内部的 depth-wise residual aggregation 模块。
换言之，本文当前关注的是“跨层表示选择”能否作为 recurrent agent 的后处理增强，而不是“跨智能体消息选择”。
这种差异也是本文讨论部分的重要结论来源：如果实验表明 depth-wise transfer 不稳定，那么更符合 MARL 任务瓶颈的后续方向可能是 agent-wise attention communication。
...
\todo{补充 ResNet 和 Transformer 的正式引用。}
...
\todo{补充 PreNorm Transformer 与 residual stream analysis 相关引用。}
...
\todo{补充 Attention Residuals 原论文与相关跨层聚合文献。}
...
\todo{补充 Decision Transformer、Transformer RL 和 LLM-inspired RL architecture 相关引用。}
