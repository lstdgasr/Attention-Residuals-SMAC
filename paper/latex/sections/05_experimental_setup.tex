...
除 agent 后处理模块外，各变体保持 learner、mixer、epsilon schedule、训练步数和测试 episode 数一致。
这种设置用于形成受控结构迁移实验，避免将训练流程差异误认为 AttnRes 模块效果。
...
主要表格均由 Sacred 日志汇总得到；其中 paired delta 表按相同 map、algorithm 和 seed 对齐 baseline 与 AttnRes-L2，以减少不同随机种子带来的比较偏差。
