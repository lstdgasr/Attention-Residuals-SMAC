...
所有变体均共享相同的输入构造、RNN hidden dimension、动作选择器、learner 和 mixer 设置。
因此，本文的比较目标不是重新调优完整 MARL pipeline，而是隔离 GRU hidden-state 后处理模块对训练稳定性和学习曲线的影响。
...
在实现中，AttnRes 内部宽度与 PyMARL 默认 RNN hidden dimension 保持一致，均为 64；dropout 设置为 0.0，RMSNorm epsilon 为 \(10^{-8}\)。
可学习 query 采用零初始化，以降低训练早期对原始 hidden representation 的扰动。
...
\end{equation}
该对照组的作用是区分“额外深度或参数量”与“depth-wise attention selection”。
如果 depth-only control 不能稳定提升性能，则说明单纯增加后处理 MLP 深度并不是可靠收益来源，也能降低将性能变化误归因于参数量增加的风险。

\subsection{AttnComm-QMIX-L2 Extension}

上述 direct transfer 变体均将 attention 放在单个 agent 内部的 depth dimension。
然而，SMAC 的核心瓶颈更常来自局部观测下的跨智能体协同。
因此，本文进一步构建 AttnComm-QMIX-L2 作为初步扩展：保持 QMIX learner、mixer 和动作选择器不变，只在 GRU hidden state 与 individual Q head 之间加入两层 agent-wise attention communication。

设同一时间步所有智能体的 GRU hidden states 为：
\begin{equation}
  H^t=[h_1^t,h_2^t,\ldots,h_n^t].
\end{equation}
第 \(\ell\) 层通信中，智能体 \(i\) 使用自身表示生成 query，并对候选通信源 \(\mathcal{N}_i\) 中的智能体表示进行 attention：
\begin{align}
  q_i^{(\ell)} &= W_q^{(\ell)}h_i^{(\ell)}, &
  k_j^{(\ell)} &= W_k^{(\ell)}h_j^{(\ell)}, &
  v_j^{(\ell)} &= W_v^{(\ell)}h_j^{(\ell)}, \\
  \alpha_{ij}^{(\ell)}
  &=
  \mathrm{softmax}_{j\in\mathcal{N}_i}
  \left(
    \frac{(q_i^{(\ell)})^\top k_j^{(\ell)}}{\sqrt{d}}
  \right), \\
  m_i^{(\ell)}
  &=
  \sum_{j\in\mathcal{N}_i}\alpha_{ij}^{(\ell)}v_j^{(\ell)} .
\end{align}
随后使用 gated residual fusion 合并原 hidden 与通信消息：
\begin{align}
  g_i^{(\ell)}
  &=
  \sigma\left(W_g^{(\ell)}[h_i^{(\ell)},m_i^{(\ell)}]\right), \\
  h_i^{(\ell+1)}
  &=
  h_i^{(\ell)}+
  g_i^{(\ell)}\odot W_o^{(\ell)}m_i^{(\ell)} .
\end{align}
经过两层通信后，\(h_i^{(2)}\) 输入 individual Q head，并继续交给原 QMIX mixer。
实现中，recurrent hidden state 仍返回通信前的 GRU hidden，避免将通信消息递归写入下一时间步，从而降低训练早期扰动。

本文比较两个通信源设置。
\code{qmix\_attncomm\_l2\_other} 使用 \(\mathcal{N}_i=\{j\mid j\neq i\}\)，强制 message 来自其他智能体。
\code{qmix\_attncomm\_l2\_self} 使用 \(\mathcal{N}_i=\{1,\ldots,n\}\)，允许模型同时选择自身和其他智能体表示。
二者用于区分收益更可能来自真实跨智能体通信，还是来自额外 self-attention 表示变换。
...
\begin{table}[H]
  ...
  \label{tab:variants}
  \footnotesize
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.30\linewidth} l >{\raggedright\arraybackslash}X c >{\raggedright\arraybackslash}p{0.18\linewidth}}
    \toprule
    ...
    \code{qmix\_attncomm\_l2\_other} & QMIX & GRU 后接 agent-wise communication & 2 & other-only communication \\
    \code{qmix\_attncomm\_l2\_self} & QMIX & GRU 后接 agent-wise communication & 2 & self-inclusive attention \\
